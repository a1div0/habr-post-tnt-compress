...
local function doc_post(uid, body)
    local zipped_body = enable_zlib and zip(body) or body
    storage:call('doc_post', {uid, zipped_body})
    return 'ok'
end

local function doc_get(uid)
    local zipped_body = storage:call('doc_get', {uid})
    return enable_zlib and unzip(zipped_body) or zipped_body
end
